...
- The app features a fully responsive UI made up a login page, a search page, and a favorites page where navigation is handled by React Router. React Context API is used to handle global state to pass favorted dogs to multiiple components as needed.

- The search page is made up of Header, Pagination (footer), Main, and Filter components which are made up of smaller components such as DogCards.
...
- Google Places API was implemented for autocomplete in the filter section, and connecting the autocomplete fields to the locations/search endpoint is processing correctly serverside but not fully complete in the UI yet.
